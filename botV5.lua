...
local hasOff = function(obj)
    -- do I have any non trump cards?
...
local canCountAllTrump = function(obj)
...
local thisPlayerOutOfTrump = function(obj, playerNumber)
    if canCountAllTrump(obj) then
        return true
    end
    local firstRound = math.floor(_G.game.thisRound / 7) * 7 + 1
    -- loop through all the rounds this hand played
...
local oppHasTrump = function(obj)
    ...
    if _G.game.rounds and #_G.game.rounds > 0 then
        ...
        local opp1hasTrump = thisPlayerOutOfTrump(obj, opponent1.ID)
        local opp2hasTrump = thisPlayerOutOfTrump(obj, opponent2.ID)
            ...
            if not hasColorLed then
                ...
                if obj.tookNest then
                    ...
                    if hasOff(obj) then
                        ...
                        if not lastOpp or thisPlayerOutOfTrump(obj, lastOpp) then
